...
public class InvoiceInspector {
   ...
   public void readInvoicesAndPayments(String invoicesFile, String paymentsFile) throws IOException {
      BufferedReader invoiceReader = new BufferedReader(
            new InputStreamReader(new FileInputStream(invoicesFile), "UTF-8"));
      String line = null;
      ...
      invoiceReader.close();
      BufferedReader paymentsReader = new BufferedReader(
            new InputStreamReader(new FileInputStream(paymentsFile), "UTF-8"));
   ...
   public void saveNewInvoices(String outputFile) throws IOException {
      ...
      for (Invoice invoice : toCollect) {
         toCollectWriter
               .write(invoice.number.toString() + "," + invoice.sum.toString() + "," + invoice.dueDate.getTime());
   ...
   public void handleInvoicesAndPaymentsFast() throws IOException {
      ...
      for (Invoice invoice : invoices) {
         Payment invoiceNumber = new Payment(invoice.number, 0);
         int paymentCounter = Algorithms.binarySearch(invoiceNumber, payments, 0, payments.length - 1);
         if (paymentCounter >= 0 && paymentCounter < payments.length) {
            Payment payment = payments[paymentCounter];
            if (invoice.sum.compareTo(payment.sum) > 0) {
               toCollect.add(new Invoice(invoice.number, invoice.sum - payment.sum, dueDateValue));
            }
